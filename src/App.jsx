              ...
              {category.featured ? (
                ...
                <div className="flex flex-col md:flex-row gap-6 items-center">
                  ...
                  <div className="flex-1">
                    ...
                    <div className={`inline-flex items-center gap-2 bg-gradient-to-r ${category.color} text-white px-6 py-3 rounded-full font-semibold group-hover:shadow-lg transition-all`}>
...
function BusinessHubPage({ onBack, onNavigateToModa }) {
  const [isVisible, setIsVisible] = useState(false);
  ...
  const businessServices = [
    ...
    },
    {
      ...
      url: 'mailto:sydney@itgirlsydcontent.com?subject=Content Creation Services'
    },
    {
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
          {businessServices.map((service, index) => {
            ...
            const Element = isModaLink ? 'button' : 'a';
            const elementProps = isModaLink 
              ? { onClick: onNavigateToModa }
              : { 
                  href: service.url,
            ...
            return (
              <Element
                key={index}
                {...elementProps}
                className={`group bg-gradient-to-br from-gray-900 to-black border-2 border-green-500/30 rounded-xl p-6 hover:border-green-400 transition-all duration-300 hover:-translate-y-2 shadow-lg ${service.glowColor} ${isModaLink ? 'w-full text-left' : ''}`}
              >
                {/* Service icon */}
                <div className="relative mb-4">
...
export default function CreatorLandingPage() {
  ...
  const [currentPage, setCurrentPage] = useState('home'); // 'home', 'business', 'beauty', 'gym', 'homehaven', 'mediakit', 'partnerships', or 'moda'
  ...
  if (currentPage === 'business') {
    return <BusinessHubPage onBack={navigateHome} onNavigateToModa={() => setCurrentPage('moda')} />;
  }
